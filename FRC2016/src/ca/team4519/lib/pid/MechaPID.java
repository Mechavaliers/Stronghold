...
public class MechaPID implements LiveWindowSendable, MechaController {
    ...
    private class PIDTask extends TimerTask {
        ...
        private MechaPID m_controller;

        public PIDTask(MechaPID mechaPID) {
            if (mechaPID == null) {
                throw new NullPointerException("Given PIDController was null");
            }
            m_controller = mechaPID;
        }
     ...
     * @param output2 The PIDOutput for second motor
     * @param period the loop time for doing calculations. This particularly effects calculations of the
     * integral and differential terms. The default is 50ms.
     */
    public MechaPID(double Kp, double Ki, double Kd, double Kf,
     ...
     * integral and differential terms. The default is 50ms.
     */
    public MechaPID(double Kp, double Ki, double Kd,
                         PIDSource source, PIDOutput output, PIDOutput output2,
                         double period) {
        this(Kp, Ki, Kd, 0.0, source, output, output2, period);
    }
    ...
    public MechaPID(double Kp, double Ki, double Kd,
                         PIDSource source, PIDOutput output, PIDOutput output2) {
        this(Kp, Ki, Kd, source, output, output2, kDefaultPeriod);
    }
    ...
    public MechaPID(double Kp, double Ki, double Kd, double Kf,
                         PIDSource source, PIDOutput output, PIDOutput output2) {
        this(Kp, Ki, Kd, Kf, source, output, output2, kDefaultPeriod);
    }
        ...
        if (enabled) {
          double input;
			...
			synchronized (this) {
                ...
                result = m_result - (gyro.getAngle() / 37.5);
